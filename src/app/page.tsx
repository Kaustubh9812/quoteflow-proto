...
export default function Home() {
  ...
  const markQuoteReady = () => {
    ...
    if (selectedQuote.checkedFollowUps.length < selectedQuote.followUpItems.length) { setError('Resolve the operator checks before marking this quote ready.'); return; }
                ...
                <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                  {selectedInboxMessage ? <div className="p-5">
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><div className="flex items-center justify-between gap-3"><div className="text-[10px] font-bold uppercase tracking-[0.13em] text-slate-400">AI confidence</div><span className="text-[10px] font-bold text-slate-600">{Math.round(selectedInboxMessage.confidence * 100)}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-teal-500" style={{ width: `${Math.round(selectedInboxMessage.confidence * 100)}%` }} /></div><p className="mt-2 text-[11px] leading-5 text-slate-500">{selectedInboxMessage.needsHumanReview ? 'Human review recommended before action.' : (selectedInboxMessage.suggestedAction || 'Review this message.')}</p></div>
                    </div>
                    ...
                    <div className="mt-5 flex flex-wrap items-center gap-2">
                      ...
                      {selectedInboxMessage.linkedQuoteId && <button type="button" onClick={() => { setSelectedQuoteId(selectedInboxMessage.linkedQuoteId); setView('quotes'); setSelectedInboxId(''); }} className="rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-[11px] font-semibold text-slate-600">Open linked quote</button>}
