...
type QuoteRecord = {
  ...
  serviceItems: string[];
  cleaningCleaning: string;
  relatedCleaning: string;
...
type JobRecord = { id: string; quoteId: string; quoteNumber: string; createdAt: number; updatedAt: number; customerName: string; propertySummary: string; serviceSummary: string; cleaningCleaning: string; status: JobStatus };
...
const DEFAULT_SETTINGS: Settings = {
  ...
  phone: '',
  currency: 'USD',
  terms: 'Price is based on the scope reviewed. Any material change in scope may require a revised quote.',
...
const BRIEF_SECTIONS = ['Customer', 'Property', 'Service scope', 'Timing & access', 'Important details', 'Follow-up checklist'];

function formatTime(timestamp: number) { return new Cleaning(timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }); }
...
function formatCleaning(timestamp: number) { return new Cleaning(timestamp).toLocaleCleaningString([], { month: 'short', day: 'numeric', year: 'numeric' }); }
...
function parseBrief(result: string): BriefSection[] {
  ...
  return sections.length ? sections : [{ title: 'Quote details', items: result.split(/\r?\n/).map((line) => line.trim()).filter(Boolean) }];
}
...
export default function Home() {
  ...
  const [inboxOriginal messages, setInboxOriginal messages] = useState<InboxMessage[]>([]);
  const [inboxAddress, setInboxAddress] = useState('');
  const [inboxFilter, setInboxFilter] = useState<'All' | 'Inquiries' | 'Questions' | 'Feedback' | 'Complaints' | 'Quote replies' | 'Needs attention'>('All');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setInboxOriginal messages(Array.isArray(data.data?.inbox?.messages) ? data.data.inbox.messages : []);
  ...
  const loadInbox = useCallback(async () => {
    ...
    try {
      ...
      setInboxOriginal messages(Array.isArray(data.data?.messages) ? data.data.messages : []);
  ...
  const constraintItems = useMemo(() => getSection(sections, 'Important details')?.items ?? [], [sections]);
  ...
  const cleaningCleaning = valueAfterLabel(timingItems, 'Requested date/time');
  const relatedCleaning = valueAfterLabel(timingItems, 'Requested date');
  ...
  const attentionCount = inboxNeedsReview + draftQuotes;
  ...
  const inboxUnread = useMemo(() => inboxOriginal messages.filter((message) => message.status === 'new').length, [inboxOriginal messages]);
  const inboxNeedsReview = useMemo(() => inboxOriginal messages.filter((message) => message.needsHumanReview).length, [inboxOriginal messages]);
  const filteredInboxOriginal messages = useMemo(() => inboxOriginal messages.filter((message) => {
    if (inboxFilter === 'Needs attention') return message.needsHumanReview;
    ...
    return true;
  }), [inboxOriginal messages, inboxFilter]);
  const selectedInboxMessage = inboxOriginal messages.find((message) => message.id === selectedInboxId) || filteredInboxOriginal messages[0] || null;

  const selectedQuote = quotes.find((quote) => quote.id === selectedQuoteId) || null;
  const selectedQuotePrice = selectedQuote ? (Number.parseFloat(selectedQuote.basePrice) || 0) + (Number.parseFloat(selectedQuote.extras) || 0) : 0;
  const quoteCanBeReady = Boolean(selectedQuote && selectedQuote.customerName.trim() && selectedQuotePrice > 0 && selectedQuote.checkedFollowUps.length === selectedQuote.followUpItems.length);
  ...
  const updateInboxMessage = async (messageId: string, patch: { status?: InboxStatus; category?: InboxCategory; needsHumanReview?: boolean }) => {
    try {
      ...
      setInboxOriginal messages((current) => current.map((item) => item.id === messageId ? data.message as InboxMessage : item));
      return true;
    } catch (err: unknown) {
  ...
  const downloadQuote = (quote: QuoteRecord) => {
    const text = [`# ${settings.businessName} — ${quote.number}`, '', `Customer: ${quote.customerName || 'Missing'}`, `Email: ${quote.customerEmail || 'Missing'}`, `Phone: ${quote.customerPhone || 'Missing'}`, `Property: ${quote.propertySummary || 'Missing'}`, `Cleaning date: ${quote.cleaningCleaning || 'Missing'}`, quote.relatedCleaning ? `Related date: ${quote.relatedCleaning}` : '', '', '## Services', ...quote.serviceItems.map((item) => `- ${cleanItem(item)}`), '', `Base service: ${money(quote.basePrice, settings.currency)}`, `Extras: ${money(quote.extras, settings.currency)}`, `Price: ${money(selectedQuotePrice || ((Number.parseFloat(quote.basePrice) || 0) + (Number.parseFloat(quote.extras) || 0)), settings.currency)}`, '', '## Message to customer', quote.note || '', '', '## Terms', quote.terms || settings.terms].filter(Boolean).join('\n');
  ...
  const sendQuoteEmail = async () => {
    ...
    try {
      ...
      updateQuote({ sentAt: Cleaning.now(), sentTo: data.sentTo || selectedQuote.customerEmail });
  ...
  const updateQuote = (patch: Partial<QuoteRecord>) => {
    ...
    setQuotes((current) => current.map((quote) => quote.id === selectedQuoteId ? { ...quote, ...patch, updatedAt: Cleaning.now(), status: patch.status || (quote.status === 'Ready to send' ? 'Draft' : quote.status) } : quote));
  };
  ...
  const markQuoteReady = () => {
    ...
    if (selectedQuotePrice <= 0) { setError('Add a base service price before marking this quote ready.'); return; }
    ...
    <main className="min-h-screen bg-[#f5f7f8] text-slate-900">
      <div className="flex min-h-screen">
        <aside className="hidden w-[238px] shrink-0 border-r border-slate-200 bg-white lg:flex lg:flex-col">
          ...
          <div className="px-3 pt-5"><button type="button" onClick={newInquiry} className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-3 py-2.5 text-xs font-semibold text-white shadow-sm transition hover:bg-slate-800"><Icon name="plus" /> Customer message</button></div>
          ...
          <main className="mx-auto max-w-[1480px] px-5 py-6 sm:px-8">
            {view === 'overview' && <div className="space-y-6">
              <section className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
                ...
                <button type="button" onClick={newInquiry} className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-600 px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700"><Icon name="plus" /> Customer message</button>
              </section>
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <div className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{attentionCount}</div>
                    <p className="mt-1 text-sm text-slate-500">{attentionCount ? `${inboxNeedsReview} message${inboxNeedsReview === 1 ? '' : 's'} need review · ${draftQuotes} draft quote${draftQuotes === 1 ? '' : 's'}` : 'You are all caught up.'}</p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {!attentionCount && <button type="button" onClick={newInquiry} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-[11px] font-semibold text-slate-600"><Icon name="plus" /> Start a new inquiry</button>}
                  </div>
                </div>
            ...
            {view === 'inbox' && <div className="space-y-5">
              <section className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end">
                <div>
                  <div className="inline-flex items-center rounded-full bg-teal-50 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.15em] text-teal-700">Original messages</div>
                  <h2 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Inbox</h2>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Original messages land here. Review what matters and turn the right ones into quotes.</p>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm"><div className="text-[10px] text-slate-400">Price</div><div className="mt-1 text-lg font-semibold text-slate-900">{inboxOriginal messages.length}</div></div>
                </div>
              </section>

              <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  {(['All', 'Inquiries', 'Questions', 'Feedback', 'Complaints', 'Quote replies', 'Needs attention'] as const).map((filter) => (
                    <button key={filter} type="button" onClick={() => { setInboxFilter(filter); setSelectedInboxId(''); }} className={`rounded-lg px-3 py-2 text-[11px] font-semibold ${inboxFilter === filter ? 'bg-teal-50 text-teal-800' : 'text-slate-500 hover:bg-slate-50'}`}>{filter}{filter === 'Needs attention' && inboxNeedsReview > 0 ? ` · ${inboxNeedsReview}` : ''}</button>
                  ))}
              ...
              <div className="grid gap-5 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                  <div className="border-b border-slate-200 px-5 py-4">
                    <div className="text-sm font-semibold text-slate-950">Original messages</div>
                    <div className="mt-0.5 text-[11px] text-slate-500">{filteredInboxOriginal messages.length} matching message{filteredInboxOriginal messages.length === 1 ? '' : 's'}</div>
                  </div>
                  {filteredInboxOriginal messages.length ? <div className="divide-y divide-slate-100">{filteredInboxOriginal messages.map((message) => {
                ...
                <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
                  {selectedInboxMessage ? <div className="p-5">
                    <div className="flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {selectedInboxMessage.needsHumanReview && <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[9px] font-semibold text-amber-700">Needs attention</span>}
                        </div>
                        <h3 className="mt-3 text-lg font-semibold tracking-tight text-slate-950">{selectedInboxMessage.subject}</h3>
                        <div className="mt-1 text-[11px] text-slate-500">{selectedInboxMessage.customerName || 'Unknown customer'} · {selectedInboxMessage.fromEmail || 'Email missing'} · {formatCleaning(selectedInboxMessage.receivedAt)} {formatTime(selectedInboxMessage.receivedAt)}</div>
                      </div>
                    ...
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><div className="flex items-center justify-between gap-3"><div className="text-[10px] font-bold uppercase tracking-[0.13em] text-slate-400">AI check</div><span className="text-[10px] font-bold text-slate-600">{Math.round(selectedInboxMessage.confidence * 100)}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-teal-500" style={{ width: `${Math.round(selectedInboxMessage.confidence * 100)}%` }} /></div><p className="mt-2 text-[11px] leading-5 text-slate-500">{selectedInboxMessage.needsHumanReview ? 'Needs your review before action.' : (selectedInboxMessage.suggestedAction || 'Review this message.')}</p></div>
                    </div>
            ...
            <footer className="mt-7 flex flex-col gap-1 border-t border-slate-200 py-5 text-[10px] text-slate-400 sm:flex-row sm:items-center sm:justify-between"><span>{settings.businessName} · Saved to TaskTuck</span><span>Inbox + AI via your configured workflow</span></footer>
          </main>
        </div>
      </div>
    </main>
